...
public class BattleSystem : MonoBehaviour
{
    ...
    IEnumerator DoRound()
    {
        ...
        else if (TeamA.activeDancers.Count > 0 && TeamB.activeDancers.Count > 0)
        {
            ...
            GameEvents.RequestFight(new FightEventData(TeamA.activeDancers[Random.Range(0, TeamA.activeDancers.Count)],TeamB.activeDancers[Random.Range(0,TeamB.activeDancers.Count)]));
        }
        else
        {
            if(TeamA.activeDancers.Count > 0)
            {
                ...
                BattleLog.print("The winner is TeamA !!!!");
            }
            else if (TeamA.activeDancers.Count == 0)
            {
                GameEvents.BattleFinished(TeamB);
                TeamB.EnableWinEffects();
                BattleLog.print("The winner is TeamB !!!!");
            }
